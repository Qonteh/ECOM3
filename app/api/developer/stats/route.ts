...
import { AnalyticsRepository, RevenueRepository, OrderRepository } from '@/lib/repositories';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [platformStats, revenueSummary, growthMetrics, orderStats, topSellers, recentRevenue] = await Promise.all([
      AnalyticsRepository.getPlatformStats(),
      RevenueRepository.getSummary(period),
      AnalyticsRepository.getGrowthMetrics(period === 'year' ? 'month' : period),

...
export async function GET(request: NextRequest) {
  try {
    ...
    const status = searchParams.get('status') as 'pending' | 'confirmed' | 'processing' | 'shipped' | 'delivered' | 'cancelled' | undefined;
    ...
    if (buyerId) {
      ...
      result = await OrderRepository.findBySeller(sellerId!, page, limit, status);
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!delivery_region || !delivery_address || !delivery_phone) {
      return NextResponse.json(
        { error: 'delivery_region, delivery_address, and delivery_phone are required' },
        { status: 400 }
      );
    }

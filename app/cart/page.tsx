                ...
                <button
                  onClick={() => {
                    if (
                      ...
                      (s.key === 'payment' && (step === 'payment' || step === 'confirmation')) ||
                  ...
                  className={cn(
                    ...
                    step === s.key
                      ...
                      : ['delivery', 'payment', 'confirmation'].indexOf(s.key) <= ['delivery', 'payment', 'confirmation'].indexOf(step)
                      ? 'text-primary'
                  ...
                  <Card key={sellerId}>
                    <CardHeader className="pb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <CardTitle className="text-base">{sellerName}</CardTitle>
                      ...
                      {sellerItems.map((item) => (
                        <div key={item.id} className="flex gap-4">
                          ...
                          <div className="text-right">
                            <p className="font-bold">{formatTZS(item.price * item.quantity)}</p>
                          </div>
                        </div>
                      ))}
                    </CardContent>
                  ...
                  <div className="grid md:grid-cols-3 gap-4">
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) => setDeliveryInfo({ ...deliveryInfo, region: value })}
                      >
                        <SelectTrigger>
                ...
                <CardContent className="space-y-6">
                  <RadioGroup value={paymentMethod} onValueChange={setPaymentMethod}>
                    <div className="grid gap-4">
                      {[
                        { value: 'mpesa', label: 'M-Pesa', description: 'Pay with Vodacom M-Pesa' },
                        { value: 'tigopesa', label: 'Tigo Pesa', description: 'Pay with Tigo Pesa' },
                        { value: 'airtelmoney', label: 'Airtel Money', description: 'Pay with Airtel Money' },
                        { value: 'halopesa', label: 'Halo Pesa', description: 'Pay with Halotel Halo Pesa' },
                  ...
                  <CardContent className="space-y-4">
                    {items.map((item) => (
                      <div key={item.id} className="flex items-center gap-3">
                        ...
                        <p className="font-medium">{formatTZS(item.price * item.quantity)}</p>
                      </div>
                    ))}
                  </CardContent>
                </Card>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-muted-foreground">
                          {deliveryInfo.address}, {deliveryInfo.ward}, {deliveryInfo.district}, {deliveryInfo.region}
                        </p>
                        {deliveryInfo.notes && (
                          <p className="text-sm text-muted-foreground mt-2">Note: {deliveryInfo.notes}</p>
                        )}
                      </div>
                    </div>
                  </CardContent>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {paymentMethod !== 'cod' && mobileNumber && (
                          <p className="text-muted-foreground">{mobileNumber}</p>
                        )}
                      </div>
                    </div>
                  </CardContent>
              ...
              <CardContent className="space-y-4">
                ...
                {step === 'cart' && (
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">Try: SOKO10 for 10% off</p>
                  </div>
                )}
                ...
                <div className="space-y-2">
                  ...
                  {deliveryFee === 0 && (
                    <p className="text-xs text-green-600">Free delivery on orders over TZS 100,000</p>
                  )}
                </div>
                ...
                {step === 'delivery' && (
                  <div className="space-y-2">
                    <Button
                      ...
                      disabled={!deliveryInfo.fullName || !deliveryInfo.phone || !deliveryInfo.region || !deliveryInfo.address}
                    >
                      Continue to Payment
                      <ChevronRight className="w-4 h-4 ml-2" />
                    </Button>
                ...
                {step === 'payment' && (
                  <div className="space-y-2">
                    ...
                    <Button variant="outline" className="w-full" onClick={() => setStep('delivery')}>
                      Back to Delivery
                    </Button>
                  </div>
                )}
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-center">
              ...
              Order number: <span className="font-medium">ORD-{Date.now().toString().slice(-6)}</span>
            </DialogDescription>
          </DialogHeader>

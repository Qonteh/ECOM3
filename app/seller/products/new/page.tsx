...
const paymentMethods = [
  ...
  { id: 'airtelmoney', name: 'Airtel Money', description: 'Airtel mobile money' },
  { id: 'halopesa', name: 'Halo Pesa', description: 'Halotel mobile money' },
  { id: 'bank', name: 'Bank Transfer', description: 'Direct bank transfer (NMB, CRDB, NBC, etc.)' },
  { id: 'cash', name: 'Cash on Delivery', description: 'Pay when you receive' },
  { id: 'installment', name: 'Lipa Pole Pole', description: 'Pay in installments' },
];

// Shipping options
const shippingOptions = [
  { id: 'pickup', name: 'Pickup Only', description: 'Buyer picks up from your location' },
  { id: 'local', name: 'Local Delivery', description: 'Within same city/region' },
  { id: 'nationwide', name: 'Nationwide Shipping', description: 'Ship anywhere in Tanzania' },
  { id: 'international', name: 'International Shipping', description: 'Ship outside Tanzania' },
];

// Product conditions with detailed descriptions
const conditions = [
  { value: 'brand_new', label: 'Brand New', description: 'Sealed, never used, with original packaging' },
  { value: 'new_open_box', label: 'New - Open Box', description: 'New item, opened but never used' },
  { value: 'like_new', label: 'Like New', description: 'Used briefly, looks and works like new' },
  { value: 'good', label: 'Good', description: 'Some signs of wear, fully functional' },
  { value: 'fair', label: 'Fair', description: 'Visible wear, still works properly' },
  { value: 'for_parts', label: 'For Parts/Repair', description: 'Not fully functional, sold as-is' },
];

// Business types for Tanzania
const businessTypes = [
  { value: 'individual', label: 'Individual Seller', description: 'Personal items, no business registration required' },
  { value: 'sole_proprietor', label: 'Sole Proprietorship', description: 'Registered business with TRA' },
  { value: 'company', label: 'Limited Company', description: 'Registered company with BRELA' },
  { value: 'cooperative', label: 'Cooperative/SACCOS', description: 'Registered cooperative society' },
];
...
export default function NewProductPage() {
  ...
  const selectedRegionDistricts = formData.region ? tanzaniaDistricts[formData.region] || [] : [];
  ...
  const togglePaymentMethod = (methodId: string) => {
    ...
    if (current.includes(methodId)) {
      setFormData({ ...formData, acceptedPayments: current.filter((m) => m !== methodId) });
    } else {
  ...
  const toggleShippingOption = (optionId: string) => {
    ...
    if (current.includes(optionId)) {
      setFormData({ ...formData, shippingOptions: current.filter((o) => o !== optionId) });
    } else {
          ...
          <TabsContent value="basic" className="space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Add up to 10 high-quality images. The first image will be the main photo.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-5 gap-4">
                  {images.map((image, index) => (
                    <div
                      key={index}
                      className="relative aspect-square bg-muted rounded-lg flex items-center justify-center border-2 border-border"
                    >
                ...
                <p className="text-xs text-muted-foreground mt-4">
                  Tip: Use clear, well-lit photos from multiple angles. Include photos of any defects or wear.
                </p>
              </CardContent>
            </Card>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {formData.title.length}/150 characters. Be specific: include brand, model, size, color
                  </p>
                </div>
              ...
              <CardContent className="space-y-4">
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    <p className="text-xs text-muted-foreground">Shows as crossed-out original price</p>
                  </div>
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setFormData({ ...formData, minimumOffer: e.target.value })}
                      disabled={!formData.allowOffers}
                    />
                    <p className="text-xs text-muted-foreground">Auto-decline offers below this %</p>
                  </div>
                </div>
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div className="flex items-center justify-between p-4 rounded-lg border border-border">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Show &quot;Negotiable&quot; badge</p>
                    </div>
                    <Switch
                ...
                <div className="p-4 rounded-lg border border-border space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Offer discounts for larger quantities</p>
                    </div>
                    <Switch
                      checked={formData.bulkPricing}
                      onCheckedChange={(checked) => setFormData({ ...formData, bulkPricing: checked })}
                    />
                  </div>
                  {formData.bulkPricing && (
                    <div className="grid sm:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <Label htmlFor="bulkMinQuantity">Minimum Quantity</Label>
                        <Input
                          id="bulkMinQuantity"
                          type="number"
                          min="2"
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="bulkPrice">Bulk Price per Unit (TZS)</Label>
                        <Input
                          id="bulkPrice"
                          type="number"
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center justify-between p-4 rounded-lg border border-border">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Allow buyers to return items</p>
                  </div>
                  <Switch
                    checked={formData.acceptReturns}
                    onCheckedChange={(checked) => setFormData({ ...formData, acceptReturns: checked })}
                  />
                </div>
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) => setFormData({ ...formData, returnPeriod: value })}
                      >
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  <div className="flex items-center space-x-3 p-3 border border-border rounded-lg">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => setFormData({ ...formData, tbsCertified: !!checked })}
                    />
                    <div>
                      <Label htmlFor="tbsCertified" className="font-medium">TBS Certified</Label>
                      <p className="text-xs text-muted-foreground">Tanzania Bureau of Standards</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-3 p-3 border border-border rounded-lg">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => setFormData({ ...formData, tfda: !!checked })}
                    />
                    <div>
                      <Label htmlFor="tfda" className="font-medium">TFDA Approved</Label>
                      <p className="text-xs text-muted-foreground">Food & Drugs Authority</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-3 p-3 border border-border rounded-lg">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => setFormData({ ...formData, halal: !!checked })}
                    />
                    <div>
                      <Label htmlFor="halal" className="font-medium">Halal Certified</Label>
                      <p className="text-xs text-muted-foreground">Islamic certification</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-3 p-3 border border-border rounded-lg">
                    <Checkbox
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Certified organic product</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-3 p-3 border border-border rounded-lg">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => setFormData({ ...formData, fairTrade: !!checked })}
                    />
                    <div>
                      <Label htmlFor="fairTrade" className="font-medium">Fair Trade</Label>
                      <p className="text-xs text-muted-foreground">Fair trade certified</p>
                    </div>
                  </div>
                </div>
              </CardContent>
          ...
          <TabsContent value="location" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Where is the product located? This helps buyers find local items.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  />
                  <p className="text-xs text-muted-foreground">
                    Help buyers find you easily by mentioning a well-known nearby location
                  </p>
                </div>
              </CardContent>
            </Card>
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div className="flex items-center justify-between p-4 rounded-lg border border-border">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Let buyers call you directly</p>
                    </div>
                    <Switch
                  ...
                  <div className="flex items-center justify-between p-4 rounded-lg border border-border">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) => setFormData({ ...formData, showWhatsApp: checked })}
                    />
                  </div>
                </div>
              ...
              <CardContent className="space-y-4">
                  ...
                  {shippingOptions.map((option) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <Checkbox checked={formData.shippingOptions.includes(option.id)} />
                        <div>
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {formData.shippingOptions.includes('local') && (
                    <div className="space-y-2">
                      <Label htmlFor="localDeliveryFee">Local Delivery Fee (TZS)</Label>
                      <Input
                        id="localDeliveryFee"
                        type="number"
                  ...
                  {formData.shippingOptions.includes('nationwide') && (
                    <div className="space-y-2">
                      <Label htmlFor="nationwideDeliveryFee">Nationwide Delivery Fee (TZS)</Label>
                      <Input
                        id="nationwideDeliveryFee"
                        type="number"
                        placeholder="e.g., 15000"
                        value={formData.nationwideDeliveryFee}
                        onChange={(e) => setFormData({ ...formData, nationwideDeliveryFee: e.target.value })}
                      />
                    </div>
                  )}
                  {formData.shippingOptions.includes('international') && (
                    <div className="space-y-2">
                      <Label htmlFor="internationalDeliveryFee">International Delivery Fee (TZS)</Label>
                      <Input
                        id="internationalDeliveryFee"
                        type="number"
                        placeholder="e.g., 50000"
                        value={formData.internationalDeliveryFee}
                        onChange={(e) => setFormData({ ...formData, internationalDeliveryFee: e.target.value })}
                      />
                    </div>
                  )}
                </div>
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label htmlFor="freeShippingMinimum">Free Shipping Above (TZS)</Label>
                    <Input
                      id="freeShippingMinimum"
                      type="number"
                      placeholder="e.g., 100000"
                      value={formData.freeShippingMinimum}
                      onChange={(e) => setFormData({ ...formData, freeShippingMinimum: e.target.value })}
                    />
                    <p className="text-xs text-muted-foreground">Leave empty if no free shipping</p>
                  </div>
                  <div className="space-y-2">
              ...
              <CardContent className="space-y-4">
                <div className="grid sm:grid-cols-2 gap-4">
                  {paymentMethods.map((method) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <Checkbox checked={formData.acceptedPayments.includes(method.id)} />
                        <div>
                    ...
                    {formData.acceptedPayments.includes('tigopesa') && (
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) => setFormData({ ...formData, tigoPesaNumber: e.target.value })}
                        />
                      </div>
                    )}
                    {formData.acceptedPayments.includes('airtelmoney') && (
                      <div className="space-y-2">
                        <Label htmlFor="airtelMoneyNumber">Airtel Money Number</Label>
                        <Input
                    ...
                    {formData.acceptedPayments.includes('halopesa') && (
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) => setFormData({ ...formData, haloPesaNumber: e.target.value })}
                        />
                      </div>
                    )}
                  </div>
                ...
                {formData.businessType !== 'individual' && (
                  <>
                    ...
                    <div className="grid sm:grid-cols-2 gap-4">
                      ...
                      {(formData.businessType === 'company' || formData.businessType === 'cooperative') && (
                        <>
                          <div className="space-y-2">
                            <Label htmlFor="brelaNumber">BRELA Registration No.</Label>
                            <Input
                              id="brelaNumber"
                              placeholder="BRELA registration number"
                              value={formData.brelaNumber}
                              onChange={(e) => setFormData({ ...formData, brelaNumber: e.target.value })}
                            />
                          </div>
                          <div className="space-y-2">
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between p-4 rounded-lg border border-border">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Appear at the top of search results (+TZS 10,000)</p>
                  </div>
                  <Switch
                ...
                <div className="flex items-center justify-between p-4 rounded-lg border border-border">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Show &quot;Urgent&quot; badge to attract quick buyers (+TZS 5,000)</p>
                  </div>
                  <Switch
                ...
                <div className="flex items-center justify-between p-4 rounded-lg border border-border">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Let buyers leave reviews after purchase</p>
                  </div>
                  <Switch
                    checked={formData.allowReviews}
                    onCheckedChange={(checked) => setFormData({ ...formData, allowReviews: checked })}
                  />
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span>TZS {(parseFloat(formData.price || '0') * 0.02).toLocaleString()}</span>
                  </div>
                  {formData.featured && (
                    <div className="flex justify-between text-sm">
                ...
                <p className="text-xs text-muted-foreground mt-4">
                  ...
                  Fee is deducted when your item sells. No upfront payment required.
                </p>
              </CardContent>
            </Card>
          </TabsContent>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-background border-t border-border p-4">
          <div className="max-w-5xl mx-auto flex items-center justify-between">
            <p className="text-sm text-muted-foreground">
              Listing Fee: <span className="font-bold">TZS {calculateListingFee().toLocaleString()}</span> (deducted on sale)
            </p>
            <div className="flex items-center gap-4">
              <Link href="/seller/products">
